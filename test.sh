...
cryptsetup luksFormat ${selected_disk}p2
cryptsetup open ${selected_disk}p2 cryptlvm
...
# Configure GRUB
sed -i 's/^GRUB_CMDLINE_LINUX=.*/GRUB_CMDLINE_LINUX="cryptdevice=\/dev\/mapper\/cryptlvm:cryptroot root=\/dev\/mapper\/cryptroot"/' /etc/default/grub
...
#umount -R /mnt
#swapoff -a
#pted, following best practices from the Arch Linux Wiki.