...
if [ "$filter_mode" = "true" ]; then
    if [ "$filter_type" = "file" ]; then
        find "$source" -type f -name "$pattern" | while read file; do
            rsync $rsync_opts "$file" "$destination"/
        done
    elif [ "$filter_type" = "-not -type f" ]; then
        find "$source" ! -type f -name "$pattern" | while read file; do
            rsync $rsync_opts "$file" "$destination"/
        done
    elif [ "$filter_type" = "dir" ]; then
        find "$source" -type d -name "$pattern" | while read dir; do
            rsync $rsync_opts "$dir" "$destination"/
        done
    elif [ "$filter_type" = "-not -type d" ]; then
        find "$source" ! -type d -name "$pattern" | while read dir; do
            rsync $rsync_opts "$dir" "$destination"/
        done
    else
